...
for ($i = 0; $i < $lightCount; $i++) {
    ...
    //Füge neuen Datenzeitpunkt ein, wenn die der Wert des letzten Eintrags sich
    //vom jetzigen unterscheidet
    if ($light->getOperatingTime() !== $lastLight->getOperatingTime() && $light->getOperatingTime() !== NULL){
        echo "OperatingTime is different" . NEXTL;
        $light->updateOperatingTime($conn, $timestamp);
    }
    
    if($light->getPower() !== $lastLight->getPower()  && $light->getPower() !== NULL){
        echo "Power is different" . NEXTL;
        $light->updatePower($conn, $timestamp);
    }
    
    if($light->getSwitchCount() !== $lastLight->getSwitchCount()  && $light->getSwitchCount() !== NULL){
        echo "SwitchCount is different" . NEXTL;
        $light->updateSwitchCount($conn, $timestamp);
    }

    if($light->getTemperature() !== $lastLight->getTemperature()  && $light->getTemperature() !== NULL){
        echo "Temperature is different" . NEXTL;
